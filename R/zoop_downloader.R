...
Zoopdownloader <- function(
  ...
  if (!file.exists(file.path(Data_folder, "zoopsFRP2018.csv")) | Redownload_data) {
    utils::download.file("https://portal.edirepository.org/nis/dataviewer?packageid=edi.269.2&entityid=d4c76f209a0653aa86bab1ff93ab9853",
  ...
  if (!file.exists(file.path(Data_folder, "bugsFRP2018.csv")) | Redownload_data) {
    utils::download.file("https://portal.edirepository.org/nis/dataviewer?packageid=edi.269.2&entityid=630f16b33a9cbf75f1989fc18690a6b3",
  ...
  zoopEnv<-zoop%>%
    dplyr::select(.data$Source, .data$Year, .data$Date, .data$Datetime, .data$Tide, .data$Station, .data$Chl, .data$Secchi, .data$Temperature, .data$BottomDepth, .data$Turbidity, .data$Microcystis, .data$pH, .data$DO, .data$SalSurf, .data$SalBott, .data$Latitude, .data$Longitude, .data$SampleID)%>%
    dplyr::distinct()

  zoop<-zoop%>%
    dplyr::select(-.data$Year, -.data$Date, -.data$Datetime, -.data$Tide, -.data$Station, -.data$Chl, -.data$Secchi, -.data$Temperature, -.data$BottomDepth, -.data$Turbidity, -.data$Microcystis, -.data$pH, -.data$DO, -.data$SalSurf, -.data$SalBott, -.data$Latitude, -.data$Longitude)
